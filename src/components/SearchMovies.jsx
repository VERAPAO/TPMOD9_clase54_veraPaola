import React, { useState, useEffect, useRef } from 'react';
import noPoster from '../assets/images/no-poster.png'; 


function SearchMovies() {

  const [movies, setMovies] = useState([]);
  const [keyword, setKeyword] = useState('action'); 
  const apiKey = '5ccc65f5'
  //const [apiKey, setApiKey] = useState('5ccc65f5&t'); 

	const searchInputRef = useRef(null);

/* -----FUNCIONES NECESARIAS PARA MANEJAR EVENTOS Y CAMPOS------- */

  	const handleChange = () => {
    	setKeyword(searchInputRef.current.value);
  	};

  const handleSubmit = (event) => {
    event.preventDefault(); 
    	setKeyword(searchInputRef.current.value); 
  };

/*-------FUNCIÓN PARA CONSUMIR LA API----------- */
  const fetchMovies = (query) => {
    const urlApiMovies = `http://www.omdbapi.com/?s=${encodeURIComponent(query)}&apikey=${apiKey}`;
    fetch(urlApiMovies)
    .then(response => response.json())
    .then(data => {
     if (data.Response === "False") {
          setMovies([]);
        } else {
          setMovies(data.Search || []); 
        }
      })
    .catch(error => console.error('Error buscando la data:', error));
  };

 
  /*-----PARA CUANDO SE MONTA EL COMPONENTE--------- */
  useEffect(() => {
	fetchMovies(keyword); 
  }, []); 

 /*----------PARA CUANDO SE ACTUALIZA------------ */
  useEffect(() => {
    fetchMovies(keyword); 
  }, [keyword]); 


  return (
    <div className="container-fluid">
      <div className="row my-4">
        <div className="col-12 col-md-6">
          {/* Barra de búsuqeda */}
          <form onSubmit={handleSubmit}>
            	<div className="form-group">
              		<label htmlFor="search">Buscar por título:</label>
					<input
						type="text"
						id="search"
						className="form-control"
						defaultValue={keyword}
						ref={searchInputRef} 
						onChange={handleChange} 
					/>
            	</div>
            		<button type="submit" className="btn btn-info">Search</button>
          </form>
        </div>
      </div>
      <div className="row">
        <div className="col-12">
          <h2>Películas para la palabra: {keyword}</h2>
        </div>
        {/* Listado de películas */}
        {movies.length > 0 ? (
          movies.map((movie, i) => (
            <div className="col-sm-6 col-md-3 my-4" key={i}>
            <div className="card shadow mb-4">
            <div className="card-header py-3">
                  <h5 className="m-0 font-weight-bold text-gray-800">{movie.Title}</h5>
            </div>
                <div className="card-body">
                  <div className="text-center">
                    <img
                      className="img-fluid px-3 px-sm-4 mt-3 mb-4"
                      src={movie.Poster !== "N/A" ? movie.Poster : noPoster}
                      alt={movie.Title}
                      style={{ width: '90%', height: '400px', objectFit: 'cover' }}
                    />
                  </div>
                  	<p>{movie.Year}</p>
                </div>
              </div>
            </div>
          ))
        ) : (
          	<div className="alert alert-warning text-center">No se encontraron películas</div>
        )}
      </div>
				{apiKey === '' && (
					<div className="alert alert-danger text-center my-4 fs-2">Eyyyy... ¿PUSISTE TU APIKEY?</div>
				)}
    </div>
  );
}

export default SearchMovies;